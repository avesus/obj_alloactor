...
struct obj_vec {
    ...
    get_alloc_vec(const uint32_t v_idx) {
        return alloc_vecs + _total_alloc_arr_size<per_level_nvec...>(n) + v_idx;
    }
    ...
    uint64_t alloc_vecs[_total_alloc_arr_size<per_level_nvec...>(
        levels + 1)] ALIGN_ATTR(CACHE_LINE_SIZE);
    uint64_t free_vecs[_total_free_arr_size(levels + 1)] ALIGN_ATTR(
        CACHE_LINE_SIZE);
